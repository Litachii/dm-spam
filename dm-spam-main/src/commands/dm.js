    ...
    try {
      ...
      if (isModerator && logChannel) {
        const startEmbed = new EmbedBuilder()
          ...
          .setDescription(`**Litachi ${botIndex + 1}** <@${targetId}> kullanıcısına ${messageCount} mesaj göndermeye başlıyor...`)
          .addFields(
            { name: 'Kullanıcı', value: `${targetUser.tag} (${targetId})`, inline: true },
            { name: 'Mesaj Sayısı', value: `${messageCount}`, inline: true },
      ...
      if (ownerLogChannel) {
        const ownerStartEmbed = new EmbedBuilder()
          ...
          .setDescription(`<@${message.author.id}> kullanıcısı, <@${targetId}> kullanıcısına **Litachi ${botIndex + 1}** botu ile spam başlattı.`)
          .addFields(
            { name: 'Hedef Kullanıcı', value: `${targetUser.tag} (${targetId})`, inline: true },
            { name: 'İsteyen Kullanıcı', value: `${message.author.tag} (${message.author.id})`, inline: true },
            { name: 'Mesaj Sayısı', value: `${messageCount}`, inline: true },
            { name: 'Bot', value: `Litachi ${botIndex + 1}`, inline: true },
      ...
      if (!dmClosed && isModerator && logChannel) {
        const successEmbed = new EmbedBuilder()
          ...
          .setDescription(`**Litachi ${botIndex + 1}** tarafından <@${targetId}> kullanıcısına ${messageCount} mesaj başarıyla gönderildi.`)
          .addFields(
            { name: 'Kullanıcı', value: `${targetUser.tag} (${targetId})`, inline: true },
            { name: 'Mesaj Sayısı', value: `${messageCount}`, inline: true },
